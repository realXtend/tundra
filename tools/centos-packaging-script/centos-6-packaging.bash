...
mv /builddir/specs/realxtend-tundra-0.0-rhel6.spec /builddir/specs/realxtend-tundra-$VER-rhel6.spec
...
sed -e "s#/naali-build#$workdir#g" /builddir/specs/realxtend-tundra-$VER-rhel6.spec > tmpfile ; mv tmpfile /builddir/specs/realxtend-tundra-$VER-rhel6.spec
sed -e 's#$HOME##g' /builddir/specs/realxtend-tundra-$VER-rhel6.spec > tmpfile ; mv tmpfile /builddir/specs/realxtend-tundra-$VER-rhel6.spec
...
sed -e "s#0.0#$VER#g" /builddir/specs/realxtend-tundra-$VER-rhel6.spec > tmpfile ; mv tmpfile /builddir/specs/realxtend-tundra-$VER-rhel6.spec
...
sed -i 's/ccache g++ -O -g /ccache g++ -O /' $naalidir/tools/build-centos6-deps.bash

#enable OGRE build
sed -i "s#private_ogre=false#private_ogre=true#g" $naalidir/tools/build-centos6-deps.bash

#build
./build-centos6-deps.bash
...
rpmbuild -bb -vv --target x86_64 --define '_topdir /rpmbuild' $rpmbuild/SPECS/realxtend-tundra-$VER-rhel6.spec

#Add timestamp if $USESTAMP is set
if [ x$USESTAMP == xtrue ]; then
	mv $rpmbuild/RPMS/x86_64/realXtend-Tundra-$VER-rhel6.x86_64.rpm $rpmbuild/RPMS/x86_64/realXtend-Tundra-$VER-$TIMESTAMP-rhel6.x86_64.rpm	
fi
