...
Section "Start Menu Shortcuts"
  ...
  CreateShortCut "$SMPROGRAMS\Tundra ${VERSION}\How to run offline demos.lnk" "$INSTDIR\dox\offlinedemos.txt" ""
  CreateShortCut "$SMPROGRAMS\Tundra ${VERSION}\Offline Demos\Animated UI.lnk" "$INSTDIR\scenes\AnimatedUI\scene.txml"
  CreateShortCut "$SMPROGRAMS\Tundra ${VERSION}\Offline Demos\Compositor.lnk" "$INSTDIR\scenes\Compositor\scene.txml"
  CreateShortCut "$SMPROGRAMS\Tundra ${VERSION}\Offline Demos\Day-Night.lnk" "$INSTDIR\scenes\DayNight\scene.txml"
  CreateShortCut "$SMPROGRAMS\Tundra ${VERSION}\Offline Demos\Sound Component.lnk" "$INSTDIR\scenes\ECSound\scene.txml"
  CreateShortCut "$SMPROGRAMS\Tundra ${VERSION}\Offline Demos\Physics.lnk" "$INSTDIR\scenes\Physics\scene.txml"
  CreateShortCut "$SMPROGRAMS\Tundra ${VERSION}\Offline Demos\Render to Texture.lnk" "$INSTDIR\scenes\RenderToTexture\scene.txml"
  CreateShortCut "$SMPROGRAMS\Tundra ${VERSION}\Offline Demos\Tooltip.lnk" "$INSTDIR\scenes\Tooltip\scene.txml"

  CreateDirectory "$SMPROGRAMS\Tundra ${VERSION}\Server-Client Demos"
  CreateShortCut "$SMPROGRAMS\Tundra ${VERSION}\How to run server-client demos.lnk" "$INSTDIR\dox\serverclientdemos.txt" ""
