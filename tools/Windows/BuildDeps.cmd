...
IF NOT EXIST "%DEPS%\boost". (
    ...
    IF %TARGET_ARCH%==x64 (
        call .\b2 -j %NUMBER_OF_PROCESSORS% address-model=64 --with-system --with-regex --with-thread --with-date_time stage
    ) ELSE (
...
IF NOT EXIST kNet.sln. (
    ...
    IF EXIST CMakeCache.txt. del /Q CMakeCache.txt
    cmake . -G %GENERATOR% -DBOOST_ROOT=%BOOST_ROOT% -DUSE_BOOST:BOOL=%USE_BOOST% -DUSE_TINYXML:BOOL=FALSE -DUSE_QT:BOOL=FALSE
    IF NOT %ERRORLEVEL%==0 GOTO :ERROR
    set BUILD_KNET=TRUE
)
...
IF NOT EXIST "%DEPS%\qtscriptgenerator\.git". (
   ...
   cd "%DEPS%"
   call git clone https://github.com/realXtend/qtscriptgenerator.git
...
IF NOT EXIST "%DEPS%\qt-solutions". (
    ...
    cd "%DEPS%"
    call git clone https://github.com/realXtend/qt-solutions.git
