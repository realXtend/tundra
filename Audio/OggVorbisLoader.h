...
bool AUDIO_API LoadOggVorbisFromFileInMemory(const u8 *fileData, size_t numBytes, std::vector<u8> &dst, bool *isStereo, bool *is16Bit, int *frequency);
...
inline bool AUDIO_API LoadOggVorbisFileToSoundBuffer(const u8 *data, size_t numBytes, SoundBuffer &dst)
{
...
/// bool AUDIO_API IdentifyOggVorbisFileInMemory(const u8 *fileData, size_t numBytes);
