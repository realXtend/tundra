...
bool AUDIO_API LoadWavFromFileInMemory(const u8 *fileData, size_t numBytes, std::vector<u8> &dst, bool *isStereo, bool *is16Bit, int *frequency);
...
inline bool AUDIO_API LoadWavFileToSoundBuffer(const u8 *data, size_t numBytes, SoundBuffer &dst)
{
...
bool AUDIO_API IdentifyWavFileInMemory(const u8 *fileData, size_t numBytes);
