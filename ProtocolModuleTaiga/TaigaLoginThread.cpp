...
TaigaLoginThread::TaigaLoginThread() : beginLogin_(false), ready_(false)
{
}
...
void TaigaLoginThread::SetupXMLRPCLogin(
...
bool TaigaLoginThread::PerformXMLRPCLogin()
{
    ...
    try
    {
        if (!authentication_)
        {
            ...
            {
                ProtocolModuleTaiga::LogWarning("Failed to read inventory: " + std::string(e.what()));
            ...
            {
                ProtocolModuleTaiga::LogWarning("Failed to read buddy list: " + std::string(e.what()));
    ...
    {
        ProtocolModuleTaiga::LogError(std::string("Login procedure threw a XMLRPCException, reason: \"") + ex.what()
            + std::string("\"."));
        ...
        try
        {
            ...
            ProtocolModuleTaiga::LogError(std::string("login_to_simulator reply returned the error message \"") +
                threadState_->errorMessage + std::string("\"."));
        }
        catch(XmlRpcException &ex)
        {
            ProtocolModuleTaiga::LogError(std::string("login_to_simulator reply did not contain an error message (") +
                ex.what() + std::string(")."));
        }
