...
void EC_BillboardWidget::InitializeOgreBillboard()
{
    ...
    {
        billboard_set_ = ogre_scene->createBillboardSet(renderer_.lock()->GetUniqueObjectName(), 1);
        assert(billboard_set_);

        material_name_ = std::string("material")+ std::string("name") + renderer_.lock()->GetUniqueObjectName();
...
void EC_BillboardWidget::Refresh()
{
    ...
    try
    {
        ...
        {
            texture_name_ = "billboardTexture" + renderer_.lock()->GetUniqueObjectName();
