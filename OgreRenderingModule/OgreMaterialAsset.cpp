                ...
                if (!ProcessBraces(line, brace_level))
                {
                    ...
                    else
                    {
                        ...
                        if ((line.substr(0, 8) == "texture ") && (line.length() > 8))
                        {
                            ...
                            QString absolute_tex_name;
                            if (QString(tex_name.c_str()).startsWith("local___"))
                                absolute_tex_name = QString(tex_name.c_str());
                            else
                                absolute_tex_name = assetAPI->LookupAssetRefToStorage(tex_name.c_str());
