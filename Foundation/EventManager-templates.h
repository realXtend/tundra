...
template <class T>
void EventManager::SendDelayedEvent(event_category_id_t category_id, event_id_t event_id, boost::shared_ptr<T> data, f64 delay)
{
   SendDelayedEvent(category_id, event_id, boost::dynamic_pointer_cast<IEventData>(data), delay);
}

template <typename T, typename U>
bool EventManager::AddSubscriber(T* subscriber, QList<U>& subscribers, int priority)
{
...
template <typename T>
bool EventManager::RegisterEventSubscriber(T* subscriber, int priority)
{
    if (!subscriber)
    {
...
template <typename T>
bool EventManager::UnregisterEventSubscriber(T* subscriber)
{
    if (!subscriber)
    {
...
template <typename T>
bool EventManager::HasEventSubscriber(T* subscriber)
{
    if (!subscriber)
    {
...
template <typename T>
bool EventManager::SendEvent(const EventSubscriber<T>& subs, event_category_id_t category_id, event_id_t event_id, IEventData* data) const
 {
