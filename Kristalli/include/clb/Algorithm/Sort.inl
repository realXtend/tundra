/** @file sort.inl
	...
	@brief Implementations of function templates for sorting algorithms.
*/

#ifndef util_Sort_inl
#define util_Sort_inl
...
void RandQuickSort(T *list, int numItems, LCG &rand, CmpFunc &cmp)
{
	...
	std::swap(list[numItems-1], list[rand.Int(0, numItems-1)]);
	...
	if (rEnd > 0) // if left side has more than one item, sort it.
		QuickSort(list, rEnd+1, cmp);
	if (numItems-rEnd>3) // if right side has more than one item, sort it.
		QuickSort(&list[rEnd+2], numItems - rEnd-2, cmp);
}
