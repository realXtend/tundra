	...
	Cons: * NOT safe for cryptography because of the easily calculatable sequential 
			...
			* Tends to have less random low-order bits (compared to the high-order bits)
	        Thus, NEVER do something like this:
	           ...
	           ulong numBetween1And10 = 1 + LCGRand.Int() % 10;

	         Instead, take into account EVERY bit of the generated number, like this:

	           ulong numBetween1And10 = 1 + (int)(10.0 * (double)LCGRand.Int()
	                                                      /(LCGRand.Max()+1.0));
	         or simply
	
	           ulong numBetween1And10 = LCGRand.Float(1.f, 10.f); */
class LCG
{
public:
	/// Initializes the generator. You probably only want to give the seed and be done with it.
	LCG(ulong seed = 1, ulong multiplier = 69621, 
		ulong increment = 0, ulong modulus = 0x7FFFFFFF /* 2^31 - 1 */)
	{
...
//	~LCG() {}
	...
	void Seed(ulong seed, ulong multiplier = 69621, ulong increment = 0, ulong modulus = 0x7FFFFFFF);
	/// Returns an integer in the range [0, MaxInt()]
	ulong Int();

	/// Returns the biggest number the generator can yield. (Which is always modulus-1)
	ulong MaxInt() const { return modulus - 1; }
