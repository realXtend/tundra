...
/** Randomized Quicksort is an optimization of Quicksort that selects a random element 
	for the pivot in each call. Makes it more difficult to deliberately cause stack 
	overflow, and improves running time for nearly sorted lists. Generally little slower 
	than Quicksort. This sort is not stable. */
template<typename T, typename CmpFunc>
void RandQuickSort(T *list, int numItems, LCG &rand, CmpFunc &cmp);
...
template<typename T> void RandQuickSort(T *list, int numItems)
{ 
	clb::LCG rand = clb::LCG(clb::Clock::Tick());
	RandQuickSort<T>(list, numItems, rand, TriCmp<T>);
}
template<typename T, typename CmpFunc> void RandQuickSort(T *list, int numItems, CmpFunc &cmp)
{ 
	clb::LCG rand = clb::LCG(clb::Clock::Tick());
	RandQuickSort<T, CmpFunc>(list, numItems, rand, cmp);
}

template<typename T> void RandQuickSort(T *list, int numItems, LCG &rand) { RandQuickSort<T>(list, numItems, rand, TriCmp<T>); }
