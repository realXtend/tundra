...
struct MsgDeleteComponent
{
	...
	MsgDeleteComponent(const char *data, size_t numBytes)
	{
		InitToDefault();
	...
	static inline const char *Name() { return "DeleteComponent"; }
	...
	{
		return 4 + 4 + 16*1 + 1 + componentTypeName.size()*1 + 1 + componentName.size()*1;
	}
