...
namespace
{
const MathLogChannel MathLogUser = 1;        ///< This log channel is free to use for user-level application messages.
const MathLogChannel MathLogInfo = 2;        ///< Successful connects and disconnects.
const MathLogChannel MathLogError = 4;       ///< All connection-related errors.
const MathLogChannel MathLogWarning = 8;       ///< All connection-related errors.
const MathLogChannel MathLogObjectAlloc = 16; ///< For debugging: prints information about object allocations an deallocations.
const MathLogChannel MathLogData = 32;       ///< For debugging: dumps information about in- and outbound packet data.
const MathLogChannel MathLogVerbose = 64;    ///< For debugging: Print detailed internal information.
const MathLogChannel MathLogWaits = 128;      ///< Logs all long performance-related waits (thread/mutex blocks) that occur in the system.
}
...
void TimeOutputDebugStringVariadic(MathLogChannel logChannel, const char *filename, int lineNumber, const char *msg, ...);

/// Prints a message to the log. Same as kNet::TimeOutputDebugStringVariadic, but does not use printf formatting.
void TimeOutputDebugString(MathLogChannel logChannel, const char *filename, int lineNumber, const char *msg);
...
void SetLogChannels(MathLogChannel logChannel);

/// Returns the currently active log channels.
MathLogChannel GetLogChannels();

/// Returns true if the specified log channel is not suppressed.
//bool IsLogChannelActive(MathLogChannel channel);
